...
The attribute LE_ProcessStep shows a BUFR decoding step while the attribute LE_Source identifies the repository.
Both LE_ProcessStep and LE_Source point to entries in the NWS Codes Registry where additional details can be documented.
The step where NCEP decoded the METAR-encoded observations and stored them in BUFR does not appear in the procedure.
Those steps that happened before the data were ingested are not documented a procedure steps.
