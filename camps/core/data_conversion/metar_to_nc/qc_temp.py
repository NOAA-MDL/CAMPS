...
def qc_temp_st(station):
    ...
    errors = check_6hour_max(
        temperature, max_temp_6_hour, station_hours, station_type)
    station_errors += errors

    errors = check_6hour_min(
        temperature, min_temp_6_hour, station_hours, station_type)
    station_errors += errors
    ...
    for i, date in enumerate(date_array):
        ...
        if (int(current_hour) % 6) == 0:
            if(len(tmp_6hours) < 3 and i != 1):
                # pdb.set_trace()
                pass
            extrema, indicies, num_missing = calc_special_max(
                tmp_6hours, six_hour_temp)
            ...
            # Check Tolerance
                ...
                new_error = qc_error(
                    ...
                    explanation="The reported synoptic "
                    "6-h max is out of tolerance, "
            ...
            # Check if garbage six_hour_temp time value
                ...
                new_error = qc_error(
                    ...
                    new_data_value=MISSING_VALUE,
                    explanation="The calculated 6-h max from the hourly "
            ...
            # Check for a rounding error in the hourlies
                    ...
                    or (
                        (temp_difference == 1 and
                            len(indicies) == 1) and
                        (indicies[0] != 0 and
                            indicies[0] != len(tmp_6hours) - 1)
            ):
                # Correct hours that were affected
                for affected_hour in indicies:
                    new_error = qc_error(
                        date_of_error=date, error_code=9902,
                        old_data_value=hourly_temp_array[(
                            i - 6) + affected_hour],
                        new_data_value=six_hour_temp,
                        explanation="The calculated 6-h max from the hourly "
                        "temperatures is greater than the reported "
            ...
            # correct six_hour_temp if the hourly is greater near the start/end
            # of the record
            elif (temp_difference > 0 and len(indicies) == 1) and \
                    (indicies[0] == 0 or
                        indicies[0] == len(tmp_6hours) - 1):
                new_error = qc_error(
                    date_of_error=date,
                    error_code=9912, old_data_value=six_hour_temp,
                    new_data_value=extrema,
                    explanation="The calculated 6-h max from the hourly "
                    "temperatures is greater than the reported "
            ...
            elif (temp_difference) >= 2 and \
                    (indicies[0] != 0 and indicies[0] != len(tmp_6hours) - 1):
                new_error = qc_error(
                    date_of_error=date,
...
def check_6hour_min(hourly_temp_array, six_hour_min_array,
    ...
    for i, date in enumerate(date_array):
        ...
        # Every 6 hours (00,06,12,18)z, do checks
        if (int(current_hour) % 6) == 0:
            extrema, indicies, num_missing = calc_special_min(
                tmp_6hours, six_hour_temp)
            temp_difference = extrema - six_hour_temp
            # Tolerance definition
            tolerance = base_tolerance + (2 * num_missing)
            ...
            # Check if garbage six_hour_temp time value
                ...
                new_error = qc_error(
                    ...
                    new_data_value=MISSING_VALUE,
                    explanation="The calculated 6-h min from the hourly "
                    "temperatures is greater than the reported "
            ...
            # Check for a rounding error in the hourlies
                ...
                or (
                (temp_difference == -1 and len(indicies) == 1) and
                (indicies[0] != 0 and indicies[0] != len(tmp_6hours) - 1)
            ):
                # Correct hours that were affected
                for affected_hour in indicies:
            ...
            # of the record
                    ...
                    (indicies[0] == 0 or indicies[0] == len(tmp_6hours) - 1)):
                new_error = qc_error(
                    date_of_error=date,
...
def calc_special_max(in_array, observed_max):
    ...
    element 1 is an array of indicies at which the the hourly max
                ...
                OR if the hourly max is only 1 over,
    ...
    for i, (dewpoint, temp) in enumerate(zip(dewpoint_arr, temperature_arr)):
        ...
        if dewpoint < -30 and dewpoint_depression > 15:
            new_error = qc_error(
                ...
                new_data_value=MISSING_VALUE,
                explanation="dewpoint is less than -30 degrees "
                "and the dewpoint depression is greater than 15 degrees"
            )
            errors.append(new_error)
        ...
        if dewpoint > temp:
            new_error = qc_error(
                ...
                new_data_value=MISSING_VALUE,
                explanation="dewpoint is greater than temperature "
            )
            errors.append(new_error)
            dewpoint_arr[i] = temp
