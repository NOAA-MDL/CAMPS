...
def apply_procedures(w_obj, procedures):
    for p in procedures:
        ...
        func(w_obj, args) # Apply the function and given args to w_obj
...
def parse_variable(entry_dict):
    ...
    if 'LeadTime' in entry_dict:
        ...
        if source == "METAR" or source == "MARINE":
            raise Exception("variable cannot have lead time and be an observation")
    ...
    if 'Vertical_Coordinate' in entry_dict:
        vert_co_dict = parse_pred.vertical_coordinate(entry_dict['Vertical_Coordinate'])
