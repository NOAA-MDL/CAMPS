...
def stats(err_list, list_len=10):
    print "----------------------------"
    print "Total number of Errors: "+str(len(err_list))
    ...
    for i in range(1,10):
        ...
        print "errors corrected during "+num_type+": "+str(num_errors)

    mostoften = Counter(map(lambda err : err.station_name, err_list)).most_common(list_len)
    print "stations most qc'd:"

    for i in mostoften:
        print i[0] + " -- " + str(i[1])
    mostoften = Counter(map(lambda err : err.error_code, err_list)).most_common(list_len)
    print "most errors of type:"
    for i in mostoften:
