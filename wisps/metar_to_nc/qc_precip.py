        ...
        for i in range(1, len(precip_1h), 6):
                ...
                if precip_24h[i] != MISSING_VALUE:
                    if corrected_24h == 4:
                        errors.append(qc_error(
                           ...
                           "24 hour precip was corrected as dew "\
                        ...
                        errors.append(qc_error(
                           ...
                           new_data_value=precip_24h[i]-precip_24_diff ,explanation=\
                           "24 hour precip was corrected as dew "\
            ...
            for j in range(0, len(p1)):
                ...
                if p1[j] != MISSING_VALUE and p1[j] > 3.75:
                    errors.append(qc_error(
                       ...
                       new_data_value=MISSING_VALUE,explanation=\
                       "1 hour precip out of range. > 3.75" \
                       ))
                    precip_1h[i+j] = MISSING_VALUE
                if present_weather[0][i+j] > 51 and present_weather[0][i+j] < 196:
                    weather_in_3h = j < 3
                    weather_in_6h = j >= 3
                    ...
                    if precip_3h[i_3h] != MISSING_VALUE and \
                            station.is_AO2_or_MANU(i+j) and precip_3h[i_3h] == 0 \
                            and weather_in_3h:
                        errors.append(qc_error(
                           station_name=station.name,date_of_error=date_3h,\
                    ...
                    if precip_6h[i_6h] != MISSING_VALUE and \
                            station.is_AO2_or_MANU(i+j) and precip_6h[i_6h] == 0:
                        errors.append(qc_error(
                           station_name=station.name,date_of_error=date_6h,\
            ...
            if (precip_6h[i_6h] != MISSING_VALUE or precip_6h[i_6h] == 0)\
                ...
                errors.append(qc_error(
                    ...
                    new_data_value=precip_3h[i_3h],explanation=\
                    "The reported present weather included precip, " \
            ...
            if station_type[i+j] >=2 and station_type[i+j]  <= 6 \
                ...
                if (precip_6h[i_6h] == 1 \
                    or precip_6h[i_6h] == 2) and not weather_in_6h:
                    # Check for fog and dewpoint depression
                    for j in range(0, len(p1)):
                        if present_weather[0][i+j] == 45 \
                                and (temperature[i+j] - dew_point[i+j] < 2): 
                            corrected_24h += 1
                            ...
                            errors.append(qc_error(
                                station_name=station.name,date_of_error=date_6h,\
                                error_code=9720,old_data_value=precip_1h[i_6h],\
                                new_data_value=0,explanation=\
                                "six hour precip is .01 or .02, dewpoint depression was " \
                    ...
                    if temp_min <= 30 and temp_max >= 35:
                        ...
                        errors.append(qc_error(
                            ...
                            new_data_value=0,explanation=\
                            "six hour precip is reported, and temperature increased " \
