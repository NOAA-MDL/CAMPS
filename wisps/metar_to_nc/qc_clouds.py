    ...
    for hour in range(0,num_hours):
        ...
        if cloud_amount[0][hour] == 9 and cloud_height[0][hour] != 0:
            errors.append(qc_error(
                    ...
                    new_data_value=0,explanation=\
                    "First group for cloud amount is POB (partially obscured sky)" 
                    ))
        ...
        if (station.name == 'KMWN' or station.name == 'KMWS') \
            ...
            errors.append(qc_error(
                        ...
                        "POB reported, but not in first cloud group" 
                        ))
        for j,tup in enumerate(zip(cloud_height, cloud_amount)):
            ...
            if height_layer[hour] > 50 and height_layer[hour] <= 100 \
                ...
                errors.append(qc_error(
                        ...
                        new_data_value=MISSING_VALUE,explanation=\
                        "cloud height is greater than 5,000 ft and less than 10,000 ft" \
                        "and not rounded to the nearest 10"
                        ))
            elif height_layer[hour] > 100 and height_layer[hour] != MISSING_VALUE \
                    and height_layer[hour] % 10 != 0 and not station.is_canadian():
                set_all_to_missing(cloud_height,hour)
                set_all_to_missing(cloud_amount,hour)
                errors.append(qc_error(
                        station_name=station.name,date_of_error=date,\
                        error_code=9501,old_data_value=height_layer[hour],\
                        new_data_value=MISSING_VALUE,explanation=\
                        "cloud height is greater than 10,000 ft, and not rounded " \
                        " to the nearest 10"
                        ))
            # Check that they're increasing through layers
                ...
                errors.append(qc_error(
                        ...
                        new_data_value=MISSING_VALUE,explanation=\
                        "Cloud heights not increasing throught groups" 
                        ))
            # Check that they're increasing through layers
                ...
                errors.append(qc_error(
                        ...
                        new_data_value=MISSING_VALUE,explanation=\
                        "Cloud amounts not increasing throught groups" 
                        ))
                set_all_to_missing(cloud_height,hour)
                set_all_to_missing(cloud_amount,hour)
            # Check that both are defined, or missing
                ...
                errors.append(qc_error(
                        ...
                        new_data_value=MISSING_VALUE,explanation=\
                        "missing cloud amount, but height is not missing\n" \
                ...
                set_all_to_missing(cloud_amount,hour)
                    ...
                    height_layer[hour] == MISSING_VALUE:
                #print str(amount_layer[hour]) +" != 9999"
                #print amount_layer[hour] != 9999
                #print height_layer[hour] == MISSING_VALUE
                errors.append(qc_error(
                        station_name=station.name,date_of_error=date,\
                        error_code=9520,old_data_value=amount_layer[hour],\
                        new_data_value=MISSING_VALUE,explanation=\
                        "missing cloud height, but cloud amount is not missing\n" \
                        "Height: "+str(height_layer[hour]) +"\n" \
                        "Amount: "+str(amount_layer[hour]) 
                        ))
                set_all_to_missing(cloud_height,hour)
